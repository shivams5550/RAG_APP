...
quiz_prompt = ChatPromptTemplate.from_messages([
    ("system", """You are a specialized AI quiz generator.
You will receive input that contains the necessary context for creating quiz questions.
The context will begin with the token: ###Context.
This context contains relevant excerpts from documents such as PDFs, textbooks, or articles.

User instructions will begin with the token: ###Question.

Your task:
- Generate exactly the number of quiz questions requested in the question.
- Use a mix of multiple choice (MCQ), true/false, and short answer questions.
- Ensure the questions are based strictly on the provided context.
- Do not mention the context or the tokens in your output.
- Format questions clearly, with numbering, and options for MCQs.
"""),
    ("user", """###Context
{context}

###Question
Generate {num_questions} quiz questions based on the above context.""")
])
...
# prompt_selector = RunnableBranch(
#     (lambda x: x.get("quiz_mode", False), quiz_prompt),
#     prompt
# )


# def append_citations(output, inputs):
#     if inputs.get("quiz_mode", False):
#         return output
#     return f"{output}\n\n**Sources:** {', '.join(inputs.get('sources', []))}"
...
#chain = RunnableLambda(retrieve_context) | prompt | llm | output_parser
#chain = citation_and_content_function | prompt_selector | llm | RunnableLambda(lambda x, inputs=None: append_citations(x, inputs))

qa_chain = RunnableLambda(retrieve_context) | prompt | llm | StrOutputParser()
#quiz_chain = RunnableLambda(retrieve_context) | quiz_prompt | llm | StrOutputParser()
